...
| | fetch 경로당 타임아웃 | **0.6초** | `RESULTS_FETCH_TIMEOUT_PER_PATH` (지연 완화) |
| | fetch 전체 타임아웃 | **1.5초** | `RESULTS_FETCH_OVERALL_TIMEOUT` (지연 완화) |
...
- **최악**: 모든 경로 0.6초씩 실패 → 1.5초 대기 → 그동안 새 회차 DB 미반영

### B. Lock 직렬화

- fetch 1회 = lock 점유 ~1.5초 (fetch 시간)
- 0.05초마다 새 fetch 시도 → lock 때문에 **스킵**
- **실질 fetch 빈도**: 1회 / 1.5초 (외부 지연 시)
...
| 항목 | 이전 | 적용 후 | 효과 |
|------|------|---------|------|
| fetch 경로당 타임아웃 | 1초 | **0.6초** | 실패 시 1초 단축, 재시도 빨라짐 |
| fetch 전체 타임아웃 | 2.5초 | **1.5초** | 전체 대기 1초 단축 |
