...
## 증상: "2만원 표시인데 5천원 배팅" (마틴 중)

### 가능 원인

1. **회차 혼동**  
   매크로 화면의 "현재 2만원"은 **다음 회차**(예: 101회) 배팅금이고, 사이트에 보이는 "5천원"은 **직전 회차**(예: 100회) 실제 배팅액일 수 있음.  
   → 사이트에 보이는 배팅 = 가장 최근 체결된 건. 매크로 표시 = 아직 배팅할 회차.

2. **서버 보정 덮어쓰기**  
   클라이언트가 2만원 POST → 서버 `get_calc_state`가 `pr == rn`으로 판단 시 `_server_calc_effective_pick_and_amount`로 5천원 계산 → **클라이언트 금액을 덮어씀**.  
   - pyo_half 마틴: 5천 = 0단계, 2만 = 3단계  
   - 서버 history가 손실 누락(또는 calc_sessions 동기화 지연)이면 0단계로 잘못 계산됨

3. **표시 vs 실제 소스**  
   - **분석기 웹**: 클라이언트 calc state 기준  
   - **매크로**: `GET /api/current-pick` 서버 응답  
   → 둘이 다를 수 있음. "2만원 표시"가 분석기 웹인지 매크로 창인지 확인 필요.

### 확인 방법

- 매크로 배팅 로그: `"배팅 실행: N회 RED/BLACK X원"` — 실제로 입력한 회차·금액
- 사이트 표시: 해당 회차의 배팅금액과 비교
- 서버 보정 로그: `[배팅연동] calc N 보정: client=X server=Y` — 덮어쓰기 발생 시 추적
