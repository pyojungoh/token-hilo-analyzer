...
| 줄run, 퐁당run | 연속 길이 시퀀스 (최신→과거). 퐁당run 첫 숫자 = 현재 퐁당 run 길이 |
| 줄%, 퐁당% | 최근 15회 기준 |
...
### 8.6 장퐁당(퐁당 run 길이) 임계값

퐁당이 몇 개까지 이어지다 끊기는지 파악하려면:

1. `python analyze_pong_run_limit.py [CSV경로]` 실행
2. 퐁당 run 길이별 다음 결과(정/꺽) 분포 확인
3. **정 52%+** 나오기 시작하는 최소 길이 → 장퐁당(끊김) 임계값 (app.py 기본 7)
4. app.py: 퐁당 2~6 → pong_w(유지), 퐁당 7+ → line_w(끊김 예상). 분석 결과에 맞게 7 조정
