...
| 열 | 설명 | 구현 요건 |
|----|------|-----------|
| **픽** | 배팅중 픽이 들어가야 함. **반픽** 또는 **스마트 반픽** 체크 시 설정한 조건에서 반대픽으로 배팅. | 예측픽(배팅중 픽) + 반픽/스마트 반픽 옵션 적용한 값으로 표시·저장. |
...
**완료 행**(actual이 정/꺽/조커인 행)의 **배팅금액·수익**은 **화면에 그릴 때** 반드시 아래 규칙만 따른다. 다른 수정 시 이 동작을 바꾸면 2열부터 마틴이 틀어지거나 금액이 꼬이는 버그가 재발한다.

| 구분 | 규칙 | 이유 |
|------|------|------|
| **완료 행 표시** | 배팅금액·수익은 **클라이언트에서 마틴게일 시뮬레이션**으로만 채운다. `roundToBetProfit`를 채우는 **한 번의 시뮬레이션 루프**(base → 마틴 테이블 → cap/currentBet 갱신) 결과만 사용한다. | 서버 history의 `h.betAmount`는 DB 저장/병합 타이밍에 따라 2열 이후부터 마틴 단계가 어긋날 수 있음. 표시는 항상 “처음부터 순서대로 마틴 시뮬레이션”한 값으로 통일해야 함. |
| **서버 h.betAmount** | 완료 행의 **표시용** 배팅금액·수익에는 **사용하지 않는다**. (저장·동기화·멈춤 판단 등에는 사용 가능.) | 위와 동일. 표에 그리는 숫자는 오직 `roundToBetProfit[rn]`만 사용. |
| **pending 행** | 대기 행은 `getBetForRound`/설정 기준으로 배팅금액 표시. 완료 행과 출처 구분 유지. | 완료 = 시뮬레이션, 대기 = 현재 설정 기준. |
...
- **반픽**: 예측픽의 반대로 항상 배팅한다.
- **스마트 반픽**: 아래 조건 중 하나라도 만족하면 반대픽으로 배팅한다.
  - **승률**: 모양승률(예측기표 모양픽 최근 50회)이 설정값 이하일 때
  - **연패+승률**: N연패 이상(설정값, 기본 3)이고 실제 경고 합산승률이 설정값 이하일 때
  - **승률방향**: 예측기표(롤링 50회 승률) 기준 저점→고점은 정픽, 고점→저점은 반대픽, 정체 시 직전 방향 참조
  - **비대칭 임계값** (옵션): 15카드 승률 추세로 판별. 승률이 **내려가는 중**이면 50% 이하에서 반픽(빠른 전환). 승률이 **올라가는 중**이면 40% 이상에서 정픽 유지(빠른 전환). 비대칭 미사용 또는 추세 불명확 시 기존 단일 임계값 사용.
  - **줄 추종** (내장): 줄 4 이상일 때 줄 방향 추종(정줄→정, 꺽줄→꺽). 줄이 끝나면 설정한 실제 경고 합산승률 임계값 따름 — 임계값 이하→반픽, 초과→정픽.
...
- **연패정지** 체크 시: 지정한 **연패 수**(조커 포함)가 나올 때까지 대기, 연패 후 **승**이 나오면 그다음 회차부터 배팅 시작.
- **합산승수** 달성 시 일시정지 후 다시 연패 대기 → 승 나오면 재시작. 반복.
- 연패 = prediction_history 기준, 조커·패 모두 포함.
- 대기 중: 자본/배팅 영역에 "대기중" 표시. 배팅 중: "현재 N승중" (합산승수, 연승 아님).
- 다른 옵션(멈춤, 목표, 마틴, 반픽 등)과 함께 동작 가능.
...
- **실행**: 계산기를 실행한다.
- **정지**: 배팅만 멈춘다. 픽은 계속 들어가고 승/패/조커는 계속 기록된다. history·pending 유지. (배팅금액 `-`, no_bet)
