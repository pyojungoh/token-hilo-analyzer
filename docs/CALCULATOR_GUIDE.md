...
- **15회 승률**은 표의 "경기승률"(예측기표 합산승률)과 다르다. **계산기 히스토리에서 완료된 마지막 15회**(멈춤 회차 포함)의 승/패만으로 계산한다. 그래서 기준값(예: 43%) 근처에서 승률이 오르내리면 멈춤/재개가 자주 바뀌어 **배팅이 띄엄띄엄** 보일 수 있다.
- **이력(히스테리시스)**: 멈춤 해제는 **기준+3% 초과**일 때만 한다(예: 43% 이하→멈춤, **46% 초과**→재개). 기준 근처에서 전환 빈도를 줄인다.
