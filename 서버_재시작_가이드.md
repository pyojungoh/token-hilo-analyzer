...
```
변수명: DATABASE_URL
값: Railway PostgreSQL 서비스의 연결 URL
```

**설정 방법:**
1. Railway 프로젝트에서 **PostgreSQL 서비스 추가**
2. PostgreSQL 서비스 → **"Variables"** 탭
3. `DATABASE_URL` 변수가 자동으로 생성됨
4. 이 값을 복사하여 메인 서비스의 **"Variables"**에 추가

또는 Railway가 자동으로 제공하는 경우:
- PostgreSQL 서비스를 추가하면 자동으로 `DATABASE_URL`이 설정됩니다
- 메인 서비스에서 PostgreSQL 서비스를 참조하도록 설정
...
## 🛠️ PostgreSQL 서비스 추가 방법 (Railway)

1. Railway 프로젝트에서 **"+ New"** 클릭
2. **"Database"** 선택
3. **"Add PostgreSQL"** 선택
4. PostgreSQL 서비스가 생성됩니다
5. PostgreSQL 서비스 → **"Variables"** 탭
6. `DATABASE_URL` 값을 복사
7. 메인 서비스 → **"Variables"** 탭
8. `DATABASE_URL` 변수 추가 (복사한 값 붙여넣기)

또는 Railway가 자동으로 연결하는 경우:
- PostgreSQL 서비스를 추가하면 메인 서비스에 자동으로 연결됩니다
- 별도 설정 없이 `DATABASE_URL`이 사용 가능합니다
