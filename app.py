...
from db import (
    init_database, get_db_connection, ensure_current_pick_table, save_game_result,
    get_calc_state, save_calc_state, _calc_state_memory, DB_AVAILABLE,
    get_color_matches_batch, ensure_database_initialized, get_recent_results_raw,
    RealDictCursor,
)
from prediction_logic import _blended_win_rate, _blended_win_rate_components, _server_recent_15_win_rate, _update_calc_paused_after_round, _calculate_calc_profit_server
from apply_logic import _apply_results_to_calcs, _server_calc_effective_pick_and_amount
from calc_handlers import _merge_calc_histories, _get_all_calc_session_ids, api_calc_state
...
def get_chunk_profile_stats(conn, profile, current_round=None, segment_type=None):
    ...
    try:
        ...
        if total < 3:
            return None
        # total 3 이상만 사용. 연패 2회↑ 1회휴식으로 억지 매칭 방어
        return {'jung_count': jung_weighted, 'kkeok_count': kkeok_weighted}
    except Exception:
        return None
...
def _scheduler_fetch_results():
    ...
    try:
        ...
        if DB_AVAILABLE and DATABASE_URL:
            # 예측픽과 동일한 results 사용 → current_pick이 예측픽과 동시에 갱신 (배팅기 지연 해소)
...
<head>
    ...
    <style>
        * {
        ...
        .calc-options-collapse { margin-top: 6px; margin-bottom: 4px; width: 100%; flex-basis: 100%; }
        .calc-options-toggle { padding: 4px 10px; font-size: 0.85em; color: #81c784; background: transparent; border: 1px solid #555; border-radius: 4px; cursor: pointer; }
        .calc-options-toggle:hover { background: #37474f; color: #a5d6a7; }
        .calc-options-collapse.collapsed .calc-options-body { display: none !important; }
        .calc-options-collapse:not(.collapsed) .calc-options-toggle { margin-bottom: 6px; }
        .calc-options-collapse.collapsed .calc-options-toggle::before { content: '▼ '; }
        .calc-options-collapse:not(.collapsed) .calc-options-toggle::before { content: '▲ '; }
...
<body>
                    ...
                    <div class="calc-dropdown collapsed" data-calc="1">
                        ...
                        <div class="calc-dropdown-body" id="calc-1-body">
                            <div class="calc-body-row">
                                ...
                                <div class="calc-options-collapse collapsed" data-calc="1">
                                    <button type="button" class="calc-options-toggle" data-calc="1" title="클릭하여 기타 옵션 펼치기/접기">기타 옵션</button>
                    ...
                    <div class="calc-dropdown collapsed" data-calc="2">
                        ...
                        <div class="calc-dropdown-body" id="calc-2-body">
                            <div class="calc-body-row">
                                ...
                                <div class="calc-options-collapse collapsed" data-calc="2">
                                    <button type="button" class="calc-options-toggle" data-calc="2" title="클릭하여 기타 옵션 펼치기/접기">기타 옵션</button>
                                    <div class="calc-options-body">
                                        <table class="calc-settings-table">
                    ...
                    <div class="calc-dropdown collapsed" data-calc="3">
                        ...
                        <div class="calc-dropdown-body" id="calc-3-body">
                            <div class="calc-body-row">
                                ...
                                <div class="calc-options-collapse collapsed" data-calc="3">
                                    <button type="button" class="calc-options-toggle" data-calc="3" title="클릭하여 기타 옵션 펼치기/접기">기타 옵션</button>
                                    <div class="calc-options-body">
                                        <table class="calc-settings-table">
    ...
    <script>
                        ...
                        if (isActualJoker) {
                            ...
                            CALC_IDS.forEach(id => {
                                ...
                                if (saved && (saved.value === '정' || saved.value === '꺽')) {
                                    ...
                                    if (useWinRateRev && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thr && noRevByMain15 && noRevByStreak5) pred = pred === '정' ? '꺽' : '정';
                                    ...
                                    if (useWinRateRev && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thr && noRevByMain15 && noRevByStreak5) betColor = betColor === '빨강' ? '검정' : '빨강';
                            ...
                            CALC_IDS.forEach(id => {
                                ...
                                if (saved && (saved.value === '정' || saved.value === '꺽')) {
                                    ...
                                    if (useWinRateRevActual && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thrActual && noRevByMain15A && noRevByStreak5A) pred = pred === '정' ? '꺽' : '정';
                                    ...
                                    if (useWinRateRevActual && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thrActual && noRevByMain15A && noRevByStreak5A) betColorActual = betColorActual === '빨강' ? '검정' : '빨강';
                        ...
                        if (isActualJoker2) {
                            CALC_IDS.forEach(id => {
                                ...
                                if (saved && (saved.value === '정' || saved.value === '꺽')) {
                                    ...
                                    if (useWinRateRev && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thr && noRevByMain152 && noRevByStreak52) pred = pred === '정' ? '꺽' : '정';
                                    ...
                                    betColor = normalizePickColor(predForRound.color);
                                    if (rev) betColor = betColor === '빨강' ? '검정' : '빨강';
                                    if (useWinRateRev && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thr && noRevByMain152 && noRevByStreak52) betColor = betColor === '빨강' ? '검정' : '빨강';
                            ...
                            CALC_IDS.forEach(id => {
                                ...
                                if (saved && (saved.value === '정' || saved.value === '꺽')) {
                                    ...
                                    if (useWinRateRevActual && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thrActual && noRevByMain153 && noRevByStreak53) pred = pred === '정' ? '꺽' : '정';
                                    ...
                                    betColorActual = normalizePickColor(predForRound.color);
                                    if (rev) betColorActual = betColorActual === '빨강' ? '검정' : '빨강';
                                    if (useWinRateRevActual && (c15 > 0 || c30 > 0 || c100 > 0) && typeof blended === 'number' && blended <= thrActual && noRevByMain153 && noRevByStreak53) betColorActual = betColorActual === '빨강' ? '검정' : '빨강';
            ...
            try {
                    ...
                    if (state.running && lastPrediction && (lastPrediction.value === '정' || lastPrediction.value === '꺽')) {
                        ...
                        if (saved && (saved.value === '정' || saved.value === '꺽')) {
                            ...
                            if (useWinRateRevCard && lowWinRate && noRevByMain15Card && noRevByStreak5Card) { bettingText = bettingText === '정' ? '꺽' : '정'; bettingIsRed = !bettingIsRed; }
        ...
        (function() {
            var key = 'calc_options_collapsed';
            try {
                var saved = localStorage.getItem(key);
                if (saved === '0' || saved === 'false') {
                    document.querySelectorAll('.calc-options-collapse').forEach(function(w) { w.classList.remove('collapsed'); });
                }
            } catch (e) {}
            document.querySelectorAll('.calc-options-toggle').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    e.stopPropagation();
                    const wrap = this.closest('.calc-options-collapse');
                    if (wrap) {
                        wrap.classList.toggle('collapsed');
                        try { localStorage.setItem(key, wrap.classList.contains('collapsed') ? '1' : '0'); } catch (e2) {}
                    }
                });
            });
        })();
        ...
        CALC_IDS.forEach(id => {
            ...
            // 마틴·마틴방식 변경 시 서버 저장 — 상단 배팅중이 설정한 마틴표와 동일하게 표시되도록
            const martingaleEl = document.getElementById('calc-' + id + '-martingale');
            const martingaleTypeEl = document.getElementById('calc-' + id + '-martingale-type');
            if (martingaleEl) martingaleEl.addEventListener('change', function() { updateCalcSummary(id); updateCalcDetail(id); try { saveCalcStateToServer({ immediate: true }); } catch (e) {} });
            if (martingaleTypeEl) martingaleTypeEl.addEventListener('change', function() { updateCalcSummary(id); updateCalcDetail(id); try { saveCalcStateToServer({ immediate: true }); } catch (e) {} });
        });
        document.addEventListener('click', function(e) {
