...
RESULTS_RESPONSE_CACHE_TTL_MS = 250  # /api/results 응답 캐시. 250ms — 120ms 폴링에서 캐시 히트율 상승, 그래프 반영 속도 개선
    ...
    <script>
        ...
        async function loadResults() {
            ...
            try {
                ...
                // 모든 카드의 색상 비교 결과 계산. 서버 graph_values 있으면 사용(클라이언트 parseCardValue 루프 생략 → 그래프 반영 속도 개선)
                ...
                if (serverGraphValues && results.length >= 16 && serverGraphValues.length >= results.length - 15) {
                    // 서버에서 정/꺽 배열 전달됨 — PREDICTION_AND_RESULT_SPEC graph_values 규격. 로컬 계산 스킵
...
def _build_results_payload_db_only(hours=24, backfill=False, results=None, ph=None):
    ...
    try:
        ...
        # 그래프용 정/꺽 배열 — 클라이언트 graphColorMatchResults 계산 생략으로 DOM 처리 속도 개선
        gv = _build_graph_values(results) if len(results) >= 16 else []
        return {
            'results': results,
            'count': len(results),
            'timestamp': datetime.now().isoformat(),
