...
def _server_win_rate_direction_zone(ph):
    ...
    최근 15경기 승률 가중: 예측 잘 맞으면(55% 이상) 반대픽 억제, 안 맞으면(45% 이하) 정픽 억제."""
    ...
    WIN_RATE_DIR_DELTA4 = 0.46   # 덜 보수적: 오름/내림 판정 더 민감
    WIN_RATE_DIR_DELTA5 = 0.51   # 덜 보수적: 고점하락/저점상승 판정 더 민감
    if len(derived) >= 4:
        recent = rates[-1]
        prev4 = rates[-4]
        ...
        if current <= WIN_RATE_LOW_BAND and is_rising:
            if rate15_pct is not None and rate15_pct <= 45:
                return 'mid_flat'  # 최근 15회 승률 낮으면 정픽 고집 완화
            return 'low_rising'
        # 고점 부근(57~60%) + 내림세 → 반대픽(high_falling)
        if current >= WIN_RATE_HIGH_BAND and is_falling:
            if rate15_pct is not None and rate15_pct >= 55:
                return 'mid_flat'  # 최근 15회 예측 잘 맞으면 반대픽 억제
            return 'high_falling'
        # 기존 ratio 기반 판정 (중간 구간) — R_LOW 0.50, R_HIGH 0.52
        if is_rising and ratio_dynamic >= 0.50:
            if rate15_pct is not None and rate15_pct <= 45:
                return 'mid_flat'
            return 'low_rising'
        if is_falling and ratio_dynamic <= 0.52:
            if rate15_pct is not None and rate15_pct >= 55:
                return 'mid_flat'
            return 'high_falling'
    if delta5 < -WIN_RATE_DIR_DELTA5 and (ratio_fixed >= 0.5 or ratio_dynamic >= 0.52):
        if rate15_pct is not None and rate15_pct >= 55:
            return 'mid_flat'
        return 'high_falling'
    if delta5 > WIN_RATE_DIR_DELTA5 and (ratio_fixed <= 0.5 or ratio_dynamic >= 0.50):
        if rate15_pct is not None and rate15_pct <= 45:
            return 'mid_flat'
        return 'low_rising'
    return 'mid_flat'
...
<body>
    ...
    <script>
        ...
        function getWinRateDirectionZone(ph) {
            ...
            var D4 = 0.46, D5 = 0.51, R_LOW = 0.50, R_HIGH = 0.52;
            if (derivedSeries.length >= 4) {
                ...
                if (current <= WIN_RATE_LOW_BAND && isRising) {
                    if (rate15Pct != null && rate15Pct <= 45) return 'mid_flat';
                    return 'low_rising';
                }
                if (current >= WIN_RATE_HIGH_BAND && isFalling) {
                    if (rate15Pct != null && rate15Pct >= 55) return 'mid_flat';
                    return 'high_falling';
                }
        ...
        function renderWinRateDirectionPanel() {
            ...
            if (derivedSeries.length > 0) {
                ...
                if (derivedSeries.length >= 6 && high != null && low != null && high > low) {
                    ...
                    } else if (isRising && ratio >= 0.50) {
                        trendZoneLabel = '오름·상위 구간';
                        refPickText = '정픽 참고';
                        refPickClass = 'color:#81c784;';
                        lastWinRateDirectionRef = '오름';
                    } else if (isFalling && ratio <= 0.52) {
                        trendZoneLabel = '내림·하위 구간';
                        refPickText = '반대픽 참고';
                        refPickClass = 'color:#e57373;';
                        lastWinRateDirectionRef = '내림';
                    } else if (delta5 < -d5 && ratio >= 0.52) {
                        trendZoneLabel = '고점 하락 구간';
                        refPickText = '반대픽 참고';
                        refPickClass = 'color:#e57373;';
                        lastWinRateDirectionRef = '내림';
                    } else if (delta5 > d5 && (current <= WIN_RATE_LOW_BAND || ratio < 0.50)) {
                    ...
                    if (refPickText === '반대픽 참고' && rate15PctPanel != null && rate15PctPanel >= 55) { refPickText = '기존 전략 유지 (최근 15회 예측 양호)'; refPickClass = 'color:#b0bec5;'; }
                    if (refPickText === '정픽 참고' && rate15PctPanel != null && rate15PctPanel <= 45) { refPickText = '기존 전략 유지 (최근 15회 예측 저조)'; refPickClass = 'color:#b0bec5;'; }
                }
            } else {
