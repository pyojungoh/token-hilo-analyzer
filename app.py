...
def _get_shape_only_pick_with_phase(results, exclude_round=None, calc_state=None):
    ...
    # 퐁당 구간: chunk_to_pong(덩어리→퐁당) 반환 시 is_pong. pong_phase는 미반환
    ...
    if is_pong:
        ...
        try:
            ...
            pass
        return None, True
    pick = _get_latest_next_pick_for_chunk(results, exclude_round=exclude_round)
    return pick if pick in ('정', '꺽') else None, False
...
def _get_win_rate_trend_from_15_cards(ph, min_diff_pct=0.5):
    """15카드(최근 회차)별 저장된 blended_win_rate와 현재 합산승률 비교 → 올라가는 중/내려가는 중 판별.
    prediction_history 각 record의 blended_win_rate = 해당 회차 시점의 실제 경고 합산승률.
    반환: 'up' | 'down' | None. None이면 추세 불명확 → 기존 단일 임계값 사용."""
    if not ph or len(ph) < 5:
        return None
    # ph: 과거→현재 순. ph[-1]=최신, blended_win_rate는 해당 회차 시점의 승률
    recent = [h.get('blended_win_rate') for h in ph[-6:-1] if h and isinstance(h.get('blended_win_rate'), (int, float))]
    if len(recent) < 3:
        return None
    recent_avg = sum(recent) / len(recent)
    current = _blended_win_rate(ph)
    if current is None:
        return None
    if current > recent_avg + min_diff_pct:
        return 'up'   # 승률이 올라가는 중 (못 맞추다 잘 맞출 때)
    if current < recent_avg - min_diff_pct:
        return 'down'  # 승률이 내려가는 중 (잘 맞추다 못 맞출 때)
    return None
...
def _get_shape_15_win_rate(ph):
    """모양(shape_pick) 최근 15회 승률. shape_pick 있는 회차만, 조커 제외."""
    if not ph:
        return None
    ...
    if len(last15) < 3:
        return None
    wins = sum(1 for h in last15 if h.get('shape_pick') == h.get('actual'))
    return round(100.0 * wins / len(last15), 1)


def _get_pong_15_win_rate(ph):
    """퐁당(pong_pick) 최근 15회 승률. pong_pick 있는 회차만, 조커 제외."""
    if not ph:
        return None
    ...
    if len(last15) < 3:
        return None
    wins = sum(1 for h in last15 if h.get('pong_pick') == h.get('actual'))
    return round(100.0 * wins / len(last15), 1)
...
def _get_prediction_picks_best(results, predicted_round, ph, shape_pong_only=False):
    """메인/메인반픽/모양/퐁당 4픽 중 15회 승률이 가장 높은 픽 선택. shape_pong_only=True면 모양·퐁당만 사용.
    ...
    main_pred = None
    if not shape_pong_only:
        try:
            filtered = [r for r in results if int(str(r.get('gameID') or '0'), 10) < predicted_round]
            if len(filtered) >= 16:
                cp = compute_prediction(filtered, ph)
                if cp and cp.get('value') in ('정', '꺽') and cp.get('round') == predicted_round:
                    main_pred = cp['value']
        except Exception:
            pass
    ...
    try:
        ...
        for session_id, state in session_states.items():
            ...
            for cid in ('1', '2', '3'):
                    ...
                    if stored_for_round and stored_for_round.get('predicted'):
                        ...
                        if c.get('prediction_picks_best') and results and len(results) >= 16:
                            try:
                                ...
                                if not shape_pong_only and isinstance(sp_cache, dict) and sp_cache.get('round') == predicted_round and sp_cache.get('calc_best_pred') and sp_cache.get('calc_best_color'):
                        ...
                        # 모양판별 옵션: shape_only와 별도. 예측기픽 사용 시 스킵
                            ...
                            try:
                                ...
                                pw = max(0, min(3, (float(c.get('pong_weight', 1)) or 1) * mul))
                                symw = max(0, min(3, (float(c.get('symmetry_weight', 1)) or 1) * mul))
                                cache_key = (latest_gid, round(sw, 2), round(cw, 2), round(pw, 2), round(symw, 2))
                                if cache_key not in _shape_hint_cache:
                                    _shape_hint_cache[cache_key] = get_shape_prediction_hint(results, ph_hint, shape_weight=sw, chunk_weight=cw, pong_weight=pw, symmetry_weight=symw)
                                hint = _shape_hint_cache[cache_key]
                                if hint and hint.get('value') in ('정', '꺽'):
                ...
                # 모양: 가장 최근 다음 픽에만 배팅 — 퐁당 구간이면 모양판별, 덩어리/줄이면 가장 최근 다음 픽. 값 없거나 픽 불일치면 no_bet
                if c.get('shape_only_latest_next_pick') and results_for_shape and len(results_for_shape) >= 16:
                    allowed_pick, _ = _get_shape_only_pick_with_phase(results_for_shape, exclude_round=pending_round, calc_state=c)
                    if not allowed_pick or allowed_pick not in ('정', '꺽') or pred_for_calc != allowed_pick:
...
def _get_card_15_color_for_latest_round(results):
    """최신 회차의 15번째 카드 색상. 예측기픽 색상 표시용. True=빨강, False=검정, None=미확인."""
...
def _compute_joker_stats(results):
    ...
    # skip_bet은 사용하지 않음 — 15번 카드가 조커일 때만 보류 (compute_prediction에서 value=None 반환)
    if count >= 2:
        out["warning"] = True
    ...
    # 조커 예고: 10회 전부터 경고 (의미 있는 선제 알림) — 보류는 15번 카드 조커일 때만
    if out.get("next_joker_rounds_est") is not None and 1 <= out["next_joker_rounds_est"] <= 10:
        out["warning"] = True
        if out["warning_reason"]:
...
def _suppress_smart_reverse_by_phase(results):
    ...
    try:
        ...
        line_runs, pong_runs = _get_line_pong_runs(use)
        head = gv[:2] if len(gv) >= 2 else None
        pong_pct, _ = _pong_line_pct(gv[:15]) if len([x for x in gv[:15] if x is True or x is False]) >= 2 else (50.0, 50.0)
        pong_prev = _pong_line_pct(gv[15:30])[0] if len(gv) >= 30 else 50.0
...
def compute_prediction(results, prediction_history, prev_symmetry_counts=None, shape_win_stats=None, chunk_profile_stats=None, use_shape_adjustments=False, shape_debug_out=None, shape_weight=1.0, chunk_weight=1.0, pong_weight=1.0, symmetry_weight=1.0):
    ...
    if isinstance(pong_chunk_debug, dict):
        ...
        if phase in ('chunk_start', 'chunk_phase', 'pong_to_chunk'):
            pong_chunk_debug['321_bottom'] = is_321_bottom
...
RESULTS_RESPONSE_CACHE_TTL_MS = 400  # /api/results 응답 캐시. apply 지연(200~500ms) 시에도 캐시 미스 감소
REFRESH_TRIGGER_THROTTLE_SEC = 0.5  # /api/results에서 _refresh_results_background 트리거 스로틀. 0.5초 — fetch·apply 경합 완화 (2단계 최적화)
...
<body>
        ...
        <div class="analysis-tabs-wrap" id="analysis-tabs-wrap">
            ...
            <div id="panel-prediction-picks" class="analysis-panel active">
                <div class="prediction-picks-cards" id="prediction-picks-cards" style="display:flex;flex-wrap:wrap;gap:16px;padding:12px 0;">
                    ...
                    <div class="pred-pick-card" data-type="pong" style="flex:1;min-width:140px;padding:16px;background:linear-gradient(135deg,#1e2a3e 0%,#1a1a2e 100%);border-radius:10px;border:2px solid #37474f;text-align:center;">
                            ...
                            <div class="calc-body-row">
                                ...
                                <div class="calc-options-wrap collapsed" data-calc="1">
                                    ...
                                    <div class="calc-options-body">
                                        <table class="calc-settings-table">
                                            <tr><td>픽/승률</td><td><label class="calc-reverse"><input type="checkbox" id="calc-1-reverse"> 반픽</label> <label><input type="checkbox" id="calc-1-smart-reverse" title="승률반픽+연패반픽+승률방향반픽 통합. 조건 하나라도 만족 시 반픽" checked> 스마트 반픽</label> <label title="승률 이 값 이하일 때 반픽">승률≤<input type="number" id="calc-1-smart-reverse-threshold" min="0" max="100" value="43" class="calc-threshold-input">%</label> <label title="이 값 이상 연패+승률≤%일 때 반픽">연패≥<input type="number" id="calc-1-smart-reverse-min-streak" min="2" max="15" value="3" class="calc-threshold-input">회</label> <label title="15카드 승률 추세로 비대칭 임계값 적용"><input type="checkbox" id="calc-1-smart-reverse-asymmetric"> 비대칭</label> <label title="승률 내려갈 때 반픽 기준">↓≤<input type="number" id="calc-1-smart-reverse-threshold-down" min="0" max="100" value="50" class="calc-threshold-input" style="width:2.5em">%</label> <label title="승률 올라갈 때 정픽 기준">↑≥<input type="number" id="calc-1-smart-reverse-threshold-up" min="0" max="100" value="40" class="calc-threshold-input" style="width:2.5em">%</label></td></tr>
                                            <tr><td>승률방향</td><td><label><input type="checkbox" id="calc-1-streak-suppress-reverse" title="4연승 또는 4연패일 때 반픽 억제 (퐁당→긴줄 전환 시 연패 감소)" checked> 줄 4 이상 반픽 억제</label> <label><input type="checkbox" id="calc-1-lock-direction-on-lose-streak" title="배팅이 연패 중일 때 방향을 바꾸지 않고 진행하던 방향 유지" checked> 연패 중 방향 고정</label></td></tr>
                                            ...
                                            <tr><td>모양판별</td><td><label><input type="checkbox" id="calc-1-shape-prediction" title="덩어리 끝 변형 허용·퐁당 가중치 등 개선된 모양 판별로 픽. 기존 모양옵션과 별도."> 모양판별 픽 사용</label> <label><input type="checkbox" id="calc-1-shape-prediction-reverse"> 모양판별반픽</label> <label title="메인 예측기표 모양판별 픽 최신 15회 승률 이 값 이하일 때 반픽">모양판별승률≤<input type="number" id="calc-1-shape-prediction-reverse-threshold" min="0" max="100" value="50" class="calc-threshold-input">%일 때 반픽</label> <label title="모양판별 계산식 내 shape/chunk/퐁당/대칭 배율(0~3, 기본 1)">shape×<input type="number" id="calc-1-shape-weight" min="0" max="3" step="0.1" value="1.1" class="calc-threshold-input" style="width:3em"> chunk×<input type="number" id="calc-1-chunk-weight" min="0" max="3" step="0.1" value="1.2" class="calc-threshold-input" style="width:3em"> 퐁당×<input type="number" id="calc-1-pong-weight" min="0" max="3" step="0.1" value="0.9" class="calc-threshold-input" style="width:3em"> 대칭×<input type="number" id="calc-1-symmetry-weight" min="0" max="3" step="0.1" value="1" class="calc-threshold-input" style="width:3em"></label></td></tr>
                        ...
                        <div class="calc-dropdown-body" id="calc-2-body">
                            <div class="calc-body-row">
                                ...
                                <div class="calc-options-wrap collapsed" data-calc="2">
                                    ...
                                    <div class="calc-options-body">
                                        <table class="calc-settings-table">
                                            <tr><td>픽/승률</td><td><label class="calc-reverse"><input type="checkbox" id="calc-2-reverse"> 반픽</label> <label><input type="checkbox" id="calc-2-smart-reverse" title="승률반픽+연패반픽+승률방향반픽 통합. 조건 하나라도 만족 시 반픽"> 스마트 반픽</label> <label title="승률 이 값 이하일 때 반픽">승률≤<input type="number" id="calc-2-smart-reverse-threshold" min="0" max="100" value="43" class="calc-threshold-input">%</label> <label title="이 값 이상 연패+승률≤%일 때 반픽">연패≥<input type="number" id="calc-2-smart-reverse-min-streak" min="2" max="15" value="3" class="calc-threshold-input">회</label> <label title="15카드 승률 추세로 비대칭 임계값 적용"><input type="checkbox" id="calc-2-smart-reverse-asymmetric"> 비대칭</label> <label title="승률 내려갈 때 반픽 기준">↓≤<input type="number" id="calc-2-smart-reverse-threshold-down" min="0" max="100" value="50" class="calc-threshold-input" style="width:2.5em">%</label> <label title="승률 올라갈 때 정픽 기준">↑≥<input type="number" id="calc-2-smart-reverse-threshold-up" min="0" max="100" value="40" class="calc-threshold-input" style="width:2.5em">%</label></td></tr>
                                            <tr><td>승률방향</td><td><label><input type="checkbox" id="calc-2-streak-suppress-reverse" title="4연승 또는 4연패일 때 반픽 억제"> 줄 4 이상 반픽 억제</label> <label><input type="checkbox" id="calc-2-lock-direction-on-lose-streak" title="배팅이 연패 중일 때 방향을 바꾸지 않고 진행하던 방향 유지" checked> 연패 중 방향 고정</label></td></tr>
                        ...
                        <div class="calc-dropdown-body" id="calc-3-body">
                            <div class="calc-body-row">
                                ...
                                <div class="calc-options-wrap collapsed" data-calc="3">
                                    ...
                                    <div class="calc-options-body">
                                        <table class="calc-settings-table">
                                            <tr><td>픽/승률</td><td><label class="calc-reverse"><input type="checkbox" id="calc-3-reverse"> 반픽</label> <label><input type="checkbox" id="calc-3-smart-reverse" title="승률반픽+연패반픽+승률방향반픽 통합. 조건 하나라도 만족 시 반픽"> 스마트 반픽</label> <label title="승률 이 값 이하일 때 반픽">승률≤<input type="number" id="calc-3-smart-reverse-threshold" min="0" max="100" value="43" class="calc-threshold-input">%</label> <label title="이 값 이상 연패+승률≤%일 때 반픽">연패≥<input type="number" id="calc-3-smart-reverse-min-streak" min="2" max="15" value="3" class="calc-threshold-input">회</label> <label title="15카드 승률 추세로 비대칭 임계값 적용"><input type="checkbox" id="calc-3-smart-reverse-asymmetric"> 비대칭</label> <label title="승률 내려갈 때 반픽 기준">↓≤<input type="number" id="calc-3-smart-reverse-threshold-down" min="0" max="100" value="50" class="calc-threshold-input" style="width:2.5em">%</label> <label title="승률 올라갈 때 정픽 기준">↑≥<input type="number" id="calc-3-smart-reverse-threshold-up" min="0" max="100" value="40" class="calc-threshold-input" style="width:2.5em">%</label></td></tr>
                                            <tr><td>승률방향</td><td><label><input type="checkbox" id="calc-3-streak-suppress-reverse" title="4연승 또는 4연패일 때 반픽 억제"> 줄 4 이상 반픽 억제</label> <label><input type="checkbox" id="calc-3-lock-direction-on-lose-streak" title="배팅이 연패 중일 때 방향을 바꾸지 않고 진행하던 방향 유지" checked> 연패 중 방향 고정</label></td></tr>
    ...
    <script>
        ...
        function savePredictionHistoryToServer(round, predicted, actual, probability, pickColor, shapePredicted) {
            // predicted가 null/undefined면 API 400 → 반복 실패 방지. 유효할 때만 전송
            var pred = (predicted === '정' || predicted === '꺽') ? predicted : ((shapePredicted === '정' || shapePredicted === '꺽') ? shapePredicted : null);
            if (pred == null || round == null || actual == null) return;
            const body = { round: round, predicted: pred, actual: actual };
        ...
        CALC_IDS.forEach(id => {
            calcState[id] = {
                ...
                reverse: false,
                smart_reverse: false,
                smart_reverse_threshold: 43,
                smart_reverse_min_streak: 3,
                smart_reverse_asymmetric: false,
                smart_reverse_threshold_down: 50,
                smart_reverse_threshold_up: 40,
                streak_suppress_reverse: false,
                lock_direction_on_lose_streak: true,
                prediction_picks_best: false,
                prediction_picks_shape_pong_only: false,
        ...
        function buildCalcPayload() {
            ...
            CALC_IDS.forEach(id => {
                ...
                payload[String(id)] = {
                    ...
                    smart_reverse_threshold: (function() { var v = (smartRevThrEl && !isNaN(parseFloat(smartRevThrEl.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrEl.value))) : 43; return typeof v === 'number' && !isNaN(v) ? v : 43; })(),
                    ...
                    smart_reverse_asymmetric: !!(document.getElementById('calc-' + id + '-smart-reverse-asymmetric') && document.getElementById('calc-' + id + '-smart-reverse-asymmetric').checked),
                    smart_reverse_threshold_down: (function() { var el = document.getElementById('calc-' + id + '-smart-reverse-threshold-down'); return (el && !isNaN(parseFloat(el.value))) ? Math.max(0, Math.min(100, parseFloat(el.value))) : 50; })(),
                    smart_reverse_threshold_up: (function() { var el = document.getElementById('calc-' + id + '-smart-reverse-threshold-up'); return (el && !isNaN(parseFloat(el.value))) ? Math.max(0, Math.min(100, parseFloat(el.value))) : 40; })(),
                    ...
                    prediction_picks_shape_pong_only: !!(document.getElementById('calc-' + id + '-prediction-picks-shape-pong-only') && document.getElementById('calc-' + id + '-prediction-picks-shape-pong-only').checked),
                    ...
                    shape_weight: (function() { var el = document.getElementById('calc-' + id + '-shape-weight'); return (el && !isNaN(parseFloat(el.value))) ? Math.max(0, Math.min(3, parseFloat(el.value))) : 1; })(),
                    ...
                    pong_weight: (function() { var el = document.getElementById('calc-' + id + '-pong-weight'); return (el && !isNaN(parseFloat(el.value))) ? Math.max(0, Math.min(3, parseFloat(el.value))) : 1; })(),
        ...
        function applyCalcsToState(calcs, serverTimeSec, restoreUi, skipIds) {
            ...
            CALC_IDS.forEach(id => {
                ...
                var smartThrRestore = (typeof c.smart_reverse_threshold === 'number' && c.smart_reverse_threshold >= 0 && c.smart_reverse_threshold <= 100) ? c.smart_reverse_threshold : ((typeof c.win_rate_threshold === 'number' && c.win_rate_threshold >= 0 && c.win_rate_threshold <= 100) ? c.win_rate_threshold : 43);
                ...
                calcState[id].smart_reverse_asymmetric = !!c.smart_reverse_asymmetric;
                calcState[id].smart_reverse_threshold_down = (typeof c.smart_reverse_threshold_down === 'number' && c.smart_reverse_threshold_down >= 0 && c.smart_reverse_threshold_down <= 100) ? c.smart_reverse_threshold_down : 50;
                calcState[id].smart_reverse_threshold_up = (typeof c.smart_reverse_threshold_up === 'number' && c.smart_reverse_threshold_up >= 0 && c.smart_reverse_threshold_up <= 100) ? c.smart_reverse_threshold_up : 40;
                ...
                calcState[id].prediction_picks_shape_pong_only = !!c.prediction_picks_shape_pong_only;
                ...
                calcState[id].shape_weight = (typeof c.shape_weight === 'number' && c.shape_weight >= 0 && c.shape_weight <= 3) ? c.shape_weight : 1;
                calcState[id].chunk_weight = (typeof c.chunk_weight === 'number' && c.chunk_weight >= 0 && c.chunk_weight <= 3) ? c.chunk_weight : 1;
                calcState[id].pong_weight = (typeof c.pong_weight === 'number' && c.pong_weight >= 0 && c.pong_weight <= 3) ? c.pong_weight : 1;
                ...
                const smartRevThrEl = document.getElementById('calc-' + id + '-smart-reverse-threshold');
                const smartRevMinEl = document.getElementById('calc-' + id + '-smart-reverse-min-streak');
                const smartRevAsymEl = document.getElementById('calc-' + id + '-smart-reverse-asymmetric');
                const smartRevThrDownEl = document.getElementById('calc-' + id + '-smart-reverse-threshold-down');
                const smartRevThrUpEl = document.getElementById('calc-' + id + '-smart-reverse-threshold-up');
                if (smartRevEl) smartRevEl.checked = !!calcState[id].smart_reverse;
                if (smartRevThrEl) smartRevThrEl.value = String(Math.round(calcState[id].smart_reverse_threshold || 43));
                if (smartRevMinEl) smartRevMinEl.value = String(Math.max(2, Math.min(15, calcState[id].smart_reverse_min_streak || 3)));
                if (smartRevAsymEl) smartRevAsymEl.checked = !!calcState[id].smart_reverse_asymmetric;
                if (smartRevThrDownEl) smartRevThrDownEl.value = String(Math.round(calcState[id].smart_reverse_threshold_down != null ? calcState[id].smart_reverse_threshold_down : 50));
                if (smartRevThrUpEl) smartRevThrUpEl.value = String(Math.round(calcState[id].smart_reverse_threshold_up != null ? calcState[id].smart_reverse_threshold_up : 40));
                ...
                var predictionPicksBestEl = document.getElementById('calc-' + id + '-prediction-picks-best');
                if (predictionPicksBestEl) predictionPicksBestEl.checked = !!calcState[id].prediction_picks_best;
                var predictionPicksShapePongOnlyEl = document.getElementById('calc-' + id + '-prediction-picks-shape-pong-only');
                if (predictionPicksShapePongOnlyEl) predictionPicksShapePongOnlyEl.checked = !!calcState[id].prediction_picks_shape_pong_only;
                ...
                if (shapeWeightEl) shapeWeightEl.value = String(calcState[id].shape_weight != null ? calcState[id].shape_weight : 1);
                ...
                if (pongWeightEl) pongWeightEl.value = String(calcState[id].pong_weight != null ? calcState[id].pong_weight : 1);
        ...
        async function loadResults() {
            ...
            try {
                ...
                // 서버 graph_values 있으면 즉시 사용 — 클라이언트 루프 제거로 5~50ms 절약
                const colorMatchResults = [];
                let graphColorMatchResults = [];
                const serverGraphValues = (data.graph_values && Array.isArray(data.graph_values)) ? data.graph_values : null;
                
                if (results.length < 16) {
                    for (let i = 0; i < displayResults.length; i++) colorMatchResults[i] = null;
                } else if (serverGraphValues && serverGraphValues.length > 0) {
                    graphColorMatchResults = serverGraphValues;
                    for (let i = 0; i < displayResults.length; i++) {
                        colorMatchResults[i] = (i < serverGraphValues.length) ? serverGraphValues[i] : null;
                    }
                } else {
                    for (let i = 0; i < displayResults.length; i++) {
                        ...
                        if (currentResult.joker || !currentGameID || results.length <= compareIndex || results[compareIndex]?.joker) {
                    ...
                    for (let i = 0; i <= results.length - 16; i++) {
                        const cur = results[i];
                        const compareIndex = i + 15;
                        if (cur?.joker || results[compareIndex]?.joker) { graphColorMatchResults.push(null); continue; }
                        const currentGameID = cur?.gameID || '';
                        const compareGameID = results[compareIndex]?.gameID || '';
                        if (!currentGameID || !compareGameID) { graphColorMatchResults.push(null); continue; }
                        const cacheKey = `${currentGameID}_${compareGameID}`;
                        if (colorMatchCache[cacheKey] !== undefined) {
        ...
        /** 15카드(최근 회차)별 blended_win_rate와 현재 합산승률 비교 → 올라가는 중/내려가는 중. 서버 _get_win_rate_trend_from_15_cards와 동일. */
        function getWinRateTrendFrom15Cards(minDiff) {
            var ph = Array.isArray(predictionHistory) ? predictionHistory.filter(function(h) { return h && typeof h === 'object'; }) : [];
            if (ph.length < 5) return null;
            var recent = [];
            for (var i = Math.max(0, ph.length - 6); i < ph.length - 1 && i >= 0; i++) {
                var bw = ph[i] && (ph[i].blended_win_rate ?? ph[i].warningWinRate);
                if (typeof bw === 'number' && !isNaN(bw)) recent.push(bw);
            }
            if (recent.length < 3) return null;
            var recentAvg = recent.reduce(function(a, b) { return a + b; }, 0) / recent.length;
            var current = getBlendedWinRate();
            if (current == null || typeof current !== 'number') return null;
            var diff = (typeof minDiff === 'number' && minDiff > 0) ? minDiff : 0.5;
            if (current > recentAvg + diff) return 'up';
        ...
        function getSmartReverseDoRev(id, blended, r15, noRevByStreak5) {
            ...
            if (typeof blended !== 'number' || isNaN(blended)) return false;
            var thrEl = document.getElementById('calc-' + id + '-smart-reverse-threshold');
            var thr = (thrEl && !isNaN(parseFloat(thrEl.value))) ? Math.max(0, Math.min(100, parseFloat(thrEl.value))) : (calcState[id] != null && typeof calcState[id].smart_reverse_threshold === 'number' ? calcState[id].smart_reverse_threshold : 43);
            var thrDownEl = document.getElementById('calc-' + id + '-smart-reverse-threshold-down');
            var thrDown = (thrDownEl && !isNaN(parseFloat(thrDownEl.value))) ? Math.max(0, Math.min(100, parseFloat(thrDownEl.value))) : (calcState[id] != null && typeof calcState[id].smart_reverse_threshold_down === 'number' ? calcState[id].smart_reverse_threshold_down : 50);
            var thrUpEl = document.getElementById('calc-' + id + '-smart-reverse-threshold-up');
            ...
            if (getLoseStreak(id) >= getLoseStreakMin(id) && blended <= thr) doRev = true;
            return doRev;
        }
        ...
        function updateCalcStatus(id) {
            ...
            if (state.running) {
                ...
                if (!!(state.streak_suppress_reverse)) statusTxt += ' · 줄4억제';
        ...
        var _updateAllCalcsDebounceMs = 80;   // 150→80ms 계산기 결과 반영 빠르게 (버벅임 최소화)
        ...
        document.querySelectorAll('.calc-run').forEach(btn => {
            btn.addEventListener('click', async function(e) {
                ...
                calcState[id].smart_reverse_threshold = (smartRevThrRun && parseFloat(smartRevThrRun.value) != null && !isNaN(parseFloat(smartRevThrRun.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrRun.value))) : 43;
                ...
                const smartRevAsymRun = document.getElementById('calc-' + id + '-smart-reverse-asymmetric');
                calcState[id].smart_reverse_asymmetric = !!(smartRevAsymRun && smartRevAsymRun.checked);
                const smartRevThrDownRun = document.getElementById('calc-' + id + '-smart-reverse-threshold-down');
                calcState[id].smart_reverse_threshold_down = (smartRevThrDownRun && !isNaN(parseFloat(smartRevThrDownRun.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrDownRun.value))) : 50;
                const smartRevThrUpRun = document.getElementById('calc-' + id + '-smart-reverse-threshold-up');
                calcState[id].smart_reverse_threshold_up = (smartRevThrUpRun && !isNaN(parseFloat(smartRevThrUpRun.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrUpRun.value))) : 40;
                ...
                var predictionPicksBestRun = document.getElementById('calc-' + id + '-prediction-picks-best');
                calcState[id].prediction_picks_best = !!(predictionPicksBestRun && predictionPicksBestRun.checked);
                var predictionPicksShapePongOnlyRun = document.getElementById('calc-' + id + '-prediction-picks-shape-pong-only');
                calcState[id].prediction_picks_shape_pong_only = !!(predictionPicksShapePongOnlyRun && predictionPicksShapePongOnlyRun.checked);
        ...
        function syncCalcOptionsFromUI(id) {
            ...
            const smartRevThrEl = document.getElementById('calc-' + id + '-smart-reverse-threshold');
            calcState[id].smart_reverse_threshold = (smartRevThrEl && !isNaN(parseFloat(smartRevThrEl.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrEl.value))) : 43;
            const smartRevMinEl = document.getElementById('calc-' + id + '-smart-reverse-min-streak');
            ...
            const smartRevAsymEl = document.getElementById('calc-' + id + '-smart-reverse-asymmetric');
            calcState[id].smart_reverse_asymmetric = !!(smartRevAsymEl && smartRevAsymEl.checked);
            const smartRevThrDownEl = document.getElementById('calc-' + id + '-smart-reverse-threshold-down');
            calcState[id].smart_reverse_threshold_down = (smartRevThrDownEl && !isNaN(parseFloat(smartRevThrDownEl.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrDownEl.value))) : 50;
            const smartRevThrUpEl = document.getElementById('calc-' + id + '-smart-reverse-threshold-up');
            calcState[id].smart_reverse_threshold_up = (smartRevThrUpEl && !isNaN(parseFloat(smartRevThrUpEl.value))) ? Math.max(0, Math.min(100, parseFloat(smartRevThrUpEl.value))) : 40;
            ...
            const predictionPicksBestEl = document.getElementById('calc-' + id + '-prediction-picks-best');
            calcState[id].prediction_picks_best = !!(predictionPicksBestEl && predictionPicksBestEl.checked);
            const predictionPicksShapePongOnlyEl = document.getElementById('calc-' + id + '-prediction-picks-shape-pong-only');
            calcState[id].prediction_picks_shape_pong_only = !!(predictionPicksShapePongOnlyEl && predictionPicksShapePongOnlyEl.checked);
            ...
            calcState[id].shape_weight = (shapeWeightEl && !isNaN(parseFloat(shapeWeightEl.value))) ? Math.max(0, Math.min(3, parseFloat(shapeWeightEl.value))) : 1;
            ...
            calcState[id].pong_weight = (pongWeightEl && !isNaN(parseFloat(pongWeightEl.value))) ? Math.max(0, Math.min(3, parseFloat(pongWeightEl.value))) : 1;
        ...
        var _lastCalcStateLoadFromResultsAt = 0;  // loadResults에서 loadCalcStateFromServer 트리거 시각 — 50ms 스로틀 (빠른 반영)
        ...
        function refreshPredictionPickOnly() {
            ...
            var jokerSkipBet = typeof lastJokerStats !== 'undefined' && lastJokerStats && lastJokerStats.skip_bet;
            ...
            var holdMsg = is15Joker ? '15번 카드 조커 · 배팅하지 마세요' : (jokerSkipBet && lastJokerStats && lastJokerStats.warning_reason ? '조커 주의 · ' + lastJokerStats.warning_reason : (jokerSkipBet ? '조커 주의 구간 · 배팅 보류' : '서버 예측 대기 중'));
        ...
        function setupIntervals() {
            ...
            var resultsInterval = isTabVisible ? 150 : 1200;   // 150ms — 결과·그래프 체크 주기 (빠른 반영)
            var calcStatusInterval = isTabVisible ? 200 : 1200; // 200ms — 픽 서버 전달
            var calcStateInterval = isTabVisible ? 150 : 5000;   // 150ms — 계산기 상태 GET (빠른 반영)
            ...
            resultsPollIntervalId = setInterval(() => {
                try {
                    ...
                    const baseInterval = allResults.length === 0 ? 150 : (anyRunning ? 100 : (criticalPhase ? 150 : 250));
    ...
    try:
        ...
        if len(results) >= 16:
            try:
                ...
                server_pred['shape_color'] = None
                server_pred['pong_color'] = None
        ...
        # 그래프용 정/꺽 시퀀스 — 클라이언트 graphColorMatchResults 루프 제거 (5~50ms 절약)
        graph_values = _build_graph_values(results) if len(results) >= 16 else []
        return {
            'results': results,
            'count': len(results),
            'timestamp': datetime.now().isoformat(),
        ...
        if DB_AVAILABLE and DATABASE_URL:
            ...
            for h in (ph or []):
                if h and h.get('shape_predicted') not in ('정', '꺽') and h.get('round') is not None:
                    ...
                    if act in ('정', '꺽'):
                        h['shape_predicted'] = act
            graph_vals = _build_graph_values(results) if len(results) >= 16 else []
            return {
                'results': results,
                'count': len(results),
                'timestamp': datetime.now().isoformat(),
...
_last_refresh_trigger_at = 0.0  # /api/results에서 refresh 트리거 스로틀 (REFRESH_TRIGGER_THROTTLE_SEC)
...
def get_results():
    """경기 결과 API. 화면 송출 보장: 캐시 유효 시 즉시 반환, 만료 시 DB에서 결과 생성."""
    try:
        ...
        # [1단계 최적화] 캐시 유효 시 DB 조회 생략. 0.1초 TTL — 10초 게임에 영향 없이 부하 대폭 감소
        now_ms = time.time() * 1000
        use_cache = (
            not result_source and not do_backfill
            and results_cache and results_cache.get('results')
            and (now_ms - last_update_time) < RESULTS_RESPONSE_CACHE_TTL_MS
        )
        if use_cache:
        ...
        # refresh 트리거: 0.5초 스로틀 — fetch·apply 경합 완화 (2단계 최적화)
        now_tr = time.time()
        if not _results_refreshing and (now_tr - _last_refresh_trigger_at) >= REFRESH_TRIGGER_THROTTLE_SEC:
...
def api_calc_state():
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            _default = {'running': False, 'started_at': 0, 'history': [], 'capital': 1000000, 'base': 10000, 'odds': 1.97, 'duration_limit': 0, 'use_duration_limit': False, 'reverse': False, 'timer_completed': False, 'smart_reverse': False, 'smart_reverse_threshold': 43, 'smart_reverse_min_streak': 3, 'smart_reverse_asymmetric': False, 'smart_reverse_threshold_down': 50, 'smart_reverse_threshold_up': 40, 'streak_suppress_reverse': False, 'prediction_picks_best': False, 'prediction_picks_shape_pong_only': False, 'shape_only_latest_next_pick': False, 'shape_prediction': False, 'last_trend_direction': None, 'martingale': False, 'martingale_type': 'pyo', 'target_enabled': False, 'target_amount': 0, 'pause_low_win_rate_enabled': False, 'pause_win_rate_threshold': 45, 'streak_wait_enabled': False, 'streak_wait_target': 3, 'streak_wait_target_wins': 15, 'streak_wait_state': 'waiting', 'streak_wait_cumulative_wins': 0, 'paused': False, 'max_win_streak_ever': 0, 'max_lose_streak_ever': 0, 'first_bet_round': 0, 'pending_round': None, 'pending_predicted': None, 'pending_prob': None, 'pending_color': None, 'pending_bet_amount': None}
            calcs = {}
            for cid in ('1', '2', '3'):
        ...
        for cid in ('1', '2', '3'):
            ...
            if isinstance(c, dict):
                ...
                out[cid] = {
                    ...
                    'smart_reverse_threshold': max(0, min(100, int(c.get('smart_reverse_threshold') or c.get('win_rate_threshold') or 43))),
                    'smart_reverse_min_streak': max(2, min(15, int(c.get('smart_reverse_min_streak') or c.get('lose_streak_reverse_min_streak') or 3))),
                    'smart_reverse_asymmetric': bool(c.get('smart_reverse_asymmetric')),
                    'smart_reverse_threshold_down': max(0, min(100, int(c.get('smart_reverse_threshold_down') or 50))),
                    'smart_reverse_threshold_up': max(0, min(100, int(c.get('smart_reverse_threshold_up') or 40))),
                    ...
                    'prediction_picks_shape_pong_only': bool(c.get('prediction_picks_shape_pong_only')),
                    ...
                    'shape_weight': max(0, min(3, float(c.get('shape_weight', 1) or 1))),
                    'chunk_weight': max(0, min(3, float(c.get('chunk_weight', 1) or 1))),
                    'pong_weight': max(0, min(3, float(c.get('pong_weight', 1) or 1))),
...
# docs/CSV_BETTING_ADVICE_GUIDE.md 참고. CSV 파일만 주어져도 밸런스·모양 분석 → 픽 권장·임계값 제안 가능.
