...
2. **좌표 수집**  
   ...
   - **레드/블랙 좌표가 정확히 안 눌릴 때**:
     1) **기기 해상도 가져오기** 버튼으로 LDPlayer 내부 해상도를 채운 뒤, 창 가로/세로와 기기 가로/세로가 맞는지 확인.
     2) `emulator_macro.py` 상단 `BET_COLOR_SWIPE_MS`를 120~150으로 늘려 터치 지속 시간을 늘림.
     3) 좌표 찾기 시 **버튼 정중앙**을 정확히 클릭. 창을 이동한 뒤에는 **창 위치 저장**을 다시 실행.
3. **문서화**  
